...
def alpso(dimensions,constraints,neqcons,xtype,x0,xmin,xmax,swarmsize,nhn,
	...
	if (mpi4py.__version__[0] == '0'):
		...
		Bcast = comm.bcast
	#end
	myswarm = list(range(myrank,swarmsize,nproc))
	if myrank != 0:
		prtOutIter = 0
		prtInnIter = 0
		fileout = 0
	...
	if not h_start:	
		...
		if myrank == 0:
			for proc in range(nproc-1):
				for i in list(p_results[proc].keys()):
	...
	if (nhm == 'dlring') or (nhm == 'slring') or (nhm == 'wheel') or (nhm == 'spatial') or (nhm == 'sfrac'):
		...
		if (nhm == 'dlring'):
			...
			#end
			if (myrank == 0):
				for i in list(sbuf.keys()):
					for i2 in list(sbuf[i].keys()):
						pdist[i,i2] = sbuf[i][i2]
						pdist[i2,i] = sbuf[i][i2]
					#end
				#end
				for proc in range(nproc-1):
					for i in list(recb[proc].keys()):
						for i2 in list(recb[proc][i].keys()):
