...
class Gradient(object):
        ...
        **Keyword arguments:**
        ...
        if (self.sens_mode.lower() == 'pgc'):
            ...
            if (mpi4py.__version__[0] == '0'):
                ...
                self.Bcast = comm.bcast
            #end
            self.mydvs = range(self.myrank,len(list(opt_problem._variables.keys())),self.nproc)
        else:
            self.myrank = 0
            self.mydvs = range(len(list(opt_problem._variables.keys())))
        #end
    ...
    def getGrad(self, x, group_ids, f, g, *args, **kwargs):
        ...
        **Arguments:**
        ...
        # 
        dfi = numpy.zeros([len(list(opt_problem._objectives.keys())),len(mydvs)],'d')
        dgi = numpy.zeros([len(list(opt_problem._constraints.keys())),len(mydvs)],'d')
        ...
        if (sens_type == 'fd'):
            ...
            for i in mydvs:
                ...
                for j in range(len(list(opt_problem._objectives.keys()))):
        ...
        df = numpy.zeros([len(list(opt_problem._objectives.keys())),len(list(opt_problem._variables.keys()))],'d')
        dg = numpy.zeros([len(list(opt_problem._constraints.keys())),len(list(opt_problem._variables.keys()))],'d')
        if (sens_mode == 'pgc'):
            if (sens_type == 'fd') or (sens_type == 'cs'):
                ...
                if myrank == 0:
                    for proc in range(self.nproc):
                        k = 0
                        for i in range(p_results[proc][0],len(list(opt_problem._variables.keys())),self.nproc):
