	...
	def __solve__(self, opt_problem={}, sens_type='FD', store_sol=True, store_hst=False, hot_start=False, disp_opts=False, sens_mode='', sens_step={}, *args, **kwargs):
		...
		**Keyword arguments:**
		...
		def soeval(x):
			...
			if self.h_start:
				...
				if (myrank == 0):
					...
					if hist_end:
						...
						df = vals['grad_obj'][0].reshape((len(list(opt_problem._objectives.keys())),len(list(opt_problem._variables.keys()))))
						dg = vals['grad_con'][0].reshape((len(list(opt_problem._constraints.keys())),len(list(opt_problem._variables.keys()))))	
					#end
				#end
				if self.pll:
					self.h_start = Bcast(self.h_start,root=0)
			...
			# Gradient Assignment
			df = df[0]
			dgg = numpy.zeros([len(list(opt_problem._variables.keys()))*2,len(list(opt_problem._variables.keys()))],'d')
			i = len(list(opt_problem._constraints.keys()))
			j = 0
			for key in list(opt_problem._variables.keys()):
				if (opt_problem._variables[key].lower != -inf): 
					if (gg[i] > 0):
