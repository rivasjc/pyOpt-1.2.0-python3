...
class Optimization(object):
    ...
    def delVar(self, i):
        ...
        **Arguments:**
        ...
        # 
        #ngroups = len(self._vargroups)
        for j in list(self._vargroups.keys()):
...
#        return self._pargroups
    ...
    def __str__(self):
        ...
        for obj in list(self._objectives.keys()):
            lines = str(self._objectives[obj]).split('\n')
            text += lines[1] + '\n'
        #end
        text += '''\n	Variables (c - continuous, i - integer, d - discrete):
        Name    Type       Value       Lower Bound  Upper Bound\n'''
        for var in list(self._variables.keys()):
        ...
        Name    Type                    Bounds\n'''
            for con in list(self._constraints.keys()):
        ...
        **Keyword arguments:**
        ...
        if disp_sols or 'solutions' in kwargs:
            if 'solutions' in kwargs:
                ...
                sol_indices = list(self._solutions.keys())
            #end
...
class Solution(Optimization):
    ...
    def __str__(self):
        ...
        if self.display_opt:
            text1 += '\n	Options:\n '
            opt_keys = list(self.options_set.keys())
            opt_keys.sort()
            for key in opt_keys:
                ns = 25-len(key)
